    ...
    void operator()(PNGHashMap *png_cache)
    {
        ...
        if (png_cache->find(fname_in) == png_cache->end()){
            try {
                ...
                png_cache->insert({fname_in, writer.getData()});
                file_out.write(&(png_cache->at(fname_in)->front()), png_cache->at(fname_in)->size());
...
private:
    ...
    // data_condition parse variable 
    std::condition_variable parse_cond_var;
...
private:
    ...
    void processQueue()
    {
        while (should_run_) {
            ...
            parse_cond_var.wait(lk, [&]{return !task_queue_.empty();});
